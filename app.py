    ...
    if uploaded is not None:
        try:
            ...
            if objects:
                ...
                with col2:
                    save_btn = st.button("Şablonu JSON kaydet")

                if save_btn:
                    cleaned_name = re.sub(r"[^a-zA-Z0-9_\-]", "_", template_name).strip("_")
                    if not cleaned_name:
                        st.error("Lütfen geçerli bir şablon adı girin.")
                    elif errors:
                        st.error("Şablon kaydedilemedi. Tekil alan uyarılarını düzeltin.")
                    else:
                        path = save_template(cleaned_name, (w, h), objects, labels)
                        st.success(f"Şablon kaydedildi: {path}")
            else:
                st.warning("Devam etmek için en az bir kutu çizin.")

        except Exception as e:
            st.error(f"PDF işlenirken hata oluştu: {e}")

with scan_tab:
    st.subheader("Şablonla tara ve Excel indir")
    template_files = list_templates()

    if not template_files:
        st.warning("Önce 'Şablon Öğret' sekmesinden bir şablon kaydedin.")
    else:
        selected_template = st.selectbox("Şablon seçin", options=template_files)
        scan_pdf = st.file_uploader("Taranacak PDF yükleyin", type=["pdf"], key="scan_pdf")

        if scan_pdf is not None:
            try:
                image = pdf_first_page_to_image(scan_pdf.read(), zoom=2.0)
                image_np = np.array(image)
                h, w = image_np.shape[:2]
                ...
                for item in template.get("fields", []):
                    ...
                    box = denormalize_box(item, w, h)
                    text = ocr_crop(image_np, box)

                    if field in {"subtotal", "vat_amount", "total_amount", "payable_amount"}:
                        row[field] = parse_amount(text)
                    else:
                        row[field] = text
                ...
                # payable_amount bulunduysa yakın çevreden subtotal/vat/total tekrar denenir
                if payable_box and row.get("payable_amount"):
                    x1, y1, x2, y2 = payable_box
                    pad_x = int((x2 - x1) * 2)
                    pad_y = int((y2 - y1) * 2)
                    near_box = (
                        max(0, x1 - pad_x),
                    ...
                    nearby_text = ocr_crop(image_np, near_box)
                    totals = extract_totals_from_nearby_text(nearby_text)
                ...
                out_name = f"tarama_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                ...
                st.success(f"Excel oluşturuldu: {out_path}")

...
def load_and_calculate_real():
    ...
    if not valid.empty:
        valid['Gun_Farki'] = (valid['Vade_Date'] - bugun).dt.days
        valid['Agirlik'] = valid['Tutar'] * valid['Gun_Farki']
        toplam_tutar = valid['Tutar'].sum()
        toplam_agirlik = valid['Agirlik'].sum()
        ort_gun_sayisi = toplam_agirlik / toplam_tutar if toplam_tutar != 0 else 0
...
def upcoming_reminders(df, days=7):
    ...
    temp_df = df.copy()
    temp_df["Vade_Date"] = pd.to_datetime(temp_df["Vade"], errors="coerce")
    temp_df["Gun_Farki"] = (temp_df["Vade_Date"] - today).dt.days
...
# --- FİLTRE UYGULAMA ---
filtered_df = df.copy()
filtered_df['Vade_Date'] = pd.to_datetime(filtered_df['Vade'], errors='coerce')
...
if tarih_araligi and len(tarih_araligi) == 2:
    start, end = pd.to_datetime(tarih_araligi[0]), pd.to_datetime(tarih_araligi[1])
        ...
        st.markdown(f"""
            <div class="metric-container">
                ...
                <div class="metric-card" style="background:#0A84FF;">
                    ...
                    <div class="value">{f_ort_vade.strftime('%d %b %Y') if not isinstance(f_ort_vade, datetime) else f_ort_vade.strftime('%d %b %Y')}</div>
                </div>
                ...
                <div class="metric-card" style="background:linear-gradient(90deg, #0A84FF, #89CFF0);">
                    <div class="icon">💵</div><div class="title">USD/TRY</div>

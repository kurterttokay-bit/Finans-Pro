...
def bootstrap_theme():
    ...
    # No theme in URL and not set in session -> render with a safe default FIRST,
    # then do a one-time JS redirect that appends ?theme=dark|light based on device preference.
    # We avoid st.stop() here because it can look like a "black screen" on first load.
    st.session_state.theme_mode = "Light"  # temporary default; will be corrected after redirect

    # Prevent repeated redirects in the same session.

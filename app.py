...
target_model = None  # fallback'i boş başlat
...
if api_key:
    ...
    try:
        ...
        st.sidebar.write("Aktif modeller:", available_models)  # debug için listeyi gör

        # Öncelik sırası: flash-latest > flash > pro-001
        ...
        elif any("1.0-pro-001" in m for m in available_models):
            target_model = [m for m in available_models if "1.0-pro-001" in m][0]

    except Exception as e:
        st.sidebar.warning(f"Model listeleme hatası: {e}")

# Eğer hiçbir model seçilemediyse uyarı ver
if not target_model:
    st.sidebar.error("Uygun Gemini modeli bulunamadı. Lütfen list_models çıktısına bak.")
